...
\[\text{Confinement Factor } (\mathcal{C}) = \frac{\sqrt{\text{R}_{max}^2 - S_t^2}}{\dots}\]

The modulated production flow (\(k_{prod}^{(t)}\)) feeding the \(dX[A]/dt\) differential equation is attenuated using the computed confinement metrics:

\[k_{prod}^{(t)} = k_{prod} \cdot \left[1.0 - f_s \cdot (1.0 - \mathcal{C})\right]\]
